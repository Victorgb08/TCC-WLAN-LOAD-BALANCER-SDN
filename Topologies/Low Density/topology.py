...
def add_aps(net):
    """Adiciona os APs à rede em uma configuração otimizada."""
    aps = []
    positions = [
        (30, 30), (50, 30), (40, 50)  # Posições ajustadas para menor sobreposição
    ]
    ...
    for i, pos in enumerate(positions):
        ...
        ap = net.addAccessPoint(
            ...
            channel=str(channel),
            mode='g',
            range=30  # Reduz o alcance para minimizar cálculos de interferência
        )
...
def topology():
    """Criação da topologia otimizada."""
    info("*** Criando a rede\n")
    net = Mininet_wifi(
    ...
    with open(MAPPINGS_FILE_PATH, "w") as f:
        for i, host in enumerate(hosts_info):
            ...
            sta = net.addStation(
                ...
                range=13  # Reduz o alcance das estações
            )
            f.write(f"{host.name} {host.mac} {host.ip}\n")
            hosts_array.append(sta)

    net.setPropagationModel(model="logDistance", exp=5.5)  # Ajusta o expoente para sinais mais simples

    info("*** Configurando nós WiFi\n")
    net.configureWifiNodes()

    info("*** Criando links\n")
    # Conecta os APs em uma topologia linear para simplificar
